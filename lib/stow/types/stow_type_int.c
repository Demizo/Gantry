 ...
 * @brief Int type for stow items
 *
 *
 */

#include <gantry/error.h>
#include <gantry/stow/types/stow_type_int.h>
 ...
 */
LOG_MODULE_REGISTER(stow_type_int, CONFIG_STOW_TYPES_LOG_LEVEL);
...
static bool validate(const union stow_constraints* constraints, data_value_t value)
{
    ASSERT(value.type == STOW_ITEM_TYPE_INT, "Unexpected value type");
...
static bool is_equal(data_value_t a, data_value_t b)
{
    ASSERT((a.type == STOW_ITEM_TYPE_INT) && (b.type == STOW_ITEM_TYPE_INT), "Unexpected value type");

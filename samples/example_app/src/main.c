...
int main(void)
{
    ...
    STOW_GET(STOW_ROLE_INTERNAL, STOW_ID_TEST_BYTES, &test_bytes);
    ...
    STOW_GET(STOW_ROLE_INTERNAL, STOW_ID_TEST_BUFFER, &test_buffer);
    ...
    STOW_GET(STOW_ROLE_INTERNAL, STOW_ID_TEST_ENUM, &test_enum);
    char* name = NULL;
    ...
    (void)STOW_SET(STOW_ROLE_INTERNAL, STOW_ID_TEST_INT, new_test_int);
    STOW_GET(STOW_ROLE_INTERNAL, STOW_ID_TEST_INT, &test_int);
    LOG_INF("Test int %d", test_int.data.int_value);
    STOW_RELEASE(STOW_ID_TEST_INT, &test_int);
    ...
    (void)STOW_SET(STOW_ROLE_INTERNAL, STOW_ID_DEVICE_NAME, new_device_name);
    STOW_GET(STOW_ROLE_INTERNAL, STOW_ID_DEVICE_NAME, &device_name);
    LOG_INF("Device name %s", device_name.data.string_value);
    STOW_RELEASE(STOW_ID_DEVICE_NAME, &device_name);
    ...
    (void)STOW_SET(STOW_ROLE_INTERNAL, STOW_ID_TEST_FLOAT, new_test_float);
    STOW_GET(STOW_ROLE_INTERNAL, STOW_ID_TEST_FLOAT, &test_float);
    LOG_INF("Test float %f", (double)test_float.data.float_value);
    STOW_RELEASE(STOW_ID_TEST_FLOAT, &test_float);
    ...
    STOW_SET(STOW_ROLE_INTERNAL, STOW_ID_TEST_BYTES, new_test_bytes);
    (void)STOW_GET(STOW_ROLE_INTERNAL, STOW_ID_TEST_BYTES, &test_bytes);
    LOG_HEXDUMP_INF(test_bytes.data.buffer_value->buf, test_bytes.data.buffer_value->len, "Test bytes");
    STOW_RELEASE(STOW_ID_TEST_BYTES, &test_bytes);
    ...
    STOW_SET(STOW_ROLE_INTERNAL, STOW_ID_TEST_BUFFER, new_test_buffer);
    (void)STOW_GET(STOW_ROLE_INTERNAL, STOW_ID_TEST_BUFFER, &test_buffer);
    LOG_HEXDUMP_INF(test_buffer.data.buffer_value->buf, test_buffer.data.buffer_value->len, "Test buffer");
    STOW_RELEASE(STOW_ID_TEST_BUFFER, &test_buffer);
    ...
    (void)STOW_SET(STOW_ROLE_INTERNAL, STOW_ID_TEST_ENUM, new_test_enum);
    STOW_GET(STOW_ROLE_INTERNAL, STOW_ID_TEST_ENUM, &test_enum);
    char* new_name = NULL;
    (void)enum_get_name_from_value(
    ...
    int i = 0;
    (void)stow_subscribe(STOW_ROLE_INTERNAL, STOW_ID_TEST_INT, &counter_handler);
    while (1)
    {
        data_value_t counter = { .type = STOW_ITEM_TYPE_INT, .data.int_value = i };
        (void)STOW_SET(STOW_ROLE_INTERNAL, STOW_ID_TEST_INT, counter);

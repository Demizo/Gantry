...
def monitor_openocd(process, event_queue):
    ...
    for line in iter(process.stdout.readline, b''):
        ...
        if "Failed to read up-channel" in decoded or "external reset detected" in decoded:
...
def run_rtt_session(args, ram_base, ram_size):
    ...
    openocd_cmd = [
        ...
        "-f", args.config,
        "-c", "init",
        "-c", f"rtt server start {args.port} 0",
        "-c", f'rtt setup {ram_base} {ram_size} "SEGGER RTT"',
        "-c", "rtt start"
    ]
...
def main():
    ...
    parser.add_argument("--device", required=True, help="Device type (e.g., nrf52840dk/nrf52840)")
    parser.add_argument("--config", required=True, help="Path to openocd.cfg")
    parser.add_argument("--port", type=int, default=8765, help="RTT Telnet port (default: 8765)")

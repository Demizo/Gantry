...
function(zds_stow_generate)
  ...
  if(NOT ARG_OUTPUT_DIR)
    set(ARG_OUTPUT_DIR ${CMAKE_CURRENT_BINARY_DIR}/zds_generated)
  endif()
...
# CONFIG_ZDS_ANALYSIS_CPPCHECK=y. Both tools also cover the module's lib/ when
# CONFIG_ZDS_ANALYSIS_CHECK_LIBRARY=y.
function(zds_register_analysis)
  set(_module_lib ${ZEPHYR_ZDS_MODULE_DIR}/lib)
  set(_app_src ${APPLICATION_SOURCE_DIR}/src)

  if(CONFIG_ZDS_ANALYSIS_CPPCHECK)
    if(CONFIG_ZDS_ANALYSIS_CHECK_LIBRARY)
      set(_cppcheck_dirs ${_module_lib} ${_app_src})
      set(_cppcheck_comment "ZDS: running cppcheck on library and application")
    else()
      set(_cppcheck_dirs ${_app_src})
      set(_cppcheck_comment "ZDS: running cppcheck on application")
    endif()

    find_program(ZDS_CPPCHECK_CMD NAMES cppcheck)
    if(ZDS_CPPCHECK_CMD)
      add_custom_target(zds_cppcheck ALL
        COMMAND ${ZDS_CPPCHECK_CMD}
          --quiet
          --inline-suppr
        ...
        VERBATIM)
      add_dependencies(app zds_cppcheck)
    else()
      message(WARNING "ZDS: cppcheck not found; skipping static analysis")
    endif()
  endif()

  set(_cocci ${ZEPHYR_ZDS_MODULE_DIR}/tools/resource_check/generated_resource_check.cocci)
  find_program(ZDS_SPATCH_CMD NAMES spatch)
  if(ZDS_SPATCH_CMD AND EXISTS "${_cocci}")
    if(CONFIG_ZDS_ANALYSIS_CHECK_LIBRARY)
      set(_spatch_comment "ZDS: running resource checker on library and application")
      add_custom_target(zds_spatch ALL
        COMMAND ${ZDS_SPATCH_CMD}
          --sp-file ${_cocci}
          --dir     ${_module_lib}
          --no-includes
          --very-quiet
      ...
      add_custom_target(zds_spatch ALL
        COMMAND ${ZDS_SPATCH_CMD}
          ...
          --very-quiet
        WORKING_DIRECTORY ${APPLICATION_SOURCE_DIR}
        COMMENT "ZDS: running resource checker on application"
        VERBATIM)
    endif()
    add_dependencies(app zds_spatch)
  elseif(NOT ZDS_SPATCH_CMD)
    message(WARNING "ZDS: spatch not found; skipping resource-leak analysis")
  endif()
endfunction()
